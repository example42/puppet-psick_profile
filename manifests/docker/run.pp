# @define psick_profile::docker::run
#
...
define psick_profile::docker::run (
  ...
  $username_prefix = $username ? {
    ''      => $psick_profile::docker::username ? {
      ''      => '',
      default => "${facts['psick_profile::docker::username']}/",
    },
    default => "${username}/",
  }
  $title_elements = split ($title, '::')
  $app = $title_elements[0]
  #  $tp_app_settings = tp_lookup($app,'settings',$::psick_profile::docker::tinydata_module,'merge')
  ...
  if $run_mode == 'service' {
    ...
    case $psick_profile::docker::module_settings['init_system'] {
      'upstart': {
    ...
    service { "${service_prefix}${sanitised_title}":
      ...
      require  => Service[$psick_profile::docker::module_settings['service_name']],
    }
  }
}
