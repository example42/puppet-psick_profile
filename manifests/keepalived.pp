...
class psick_profile::keepalived (
  ...
  if $manage {
    ...
    # example: psick_profile::keepalived::config_dir_source: "puppet:///modules/psick_profile/keepalived/services/"
    ...
    $virtualservers.each | String $vs | {
      ...
      psick_profile::keepalived::vrrp { $vs:
        ensure       => $ensure,
        vip          => $server_role['vip'],
      ...
      if $server_role['port'] =~ Iterable {
        $server_role['port'].each | String $port_type , Integer $port | {
          psick_profile::keepalived::vs { "${vs}-${port}":
            ensure       => $ensure,
            vip          => $server_role['vip'],
            vip_port     => $port,
            user_options => $options,
