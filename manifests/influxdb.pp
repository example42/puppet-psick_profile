...
#     psick_profile::influxdb::no_noop: true
#
    ...
    case $module {
      'tp_profile': {
        ...
        $databases_hash.each | $k,$v | {
          psick_profile::influxdb::database { $k:
            * => $v,
          }
        }
        $users_hash.each | $k,$v | {
          psick_profile::influxdb::user { $k:
            * => $v,
          }
        }
        $grants_hash.each | $k,$v | {
          psick_profile::influxdb::grant { $k:
            * => $v,
          }
        }
      }
