...
class psick_profile::icinga2 (
  ...
  if $manage {
    ...
    if $influxdb_manage and $is_server {
      ...
      psick_profile::influxdb::user { 'icinga2':
        ...
        server_user     => $influxdb_settings['influxdb_user'],
        server_password => $influxdb_settings['influxdb_password'],
        require         => Psick_profile::Influxdb::Database['icinga2'],
      }
      psick_profile::influxdb::grant { 'icinga2':
        user            => $influxdb_settings['user'],
        database        => $influxdb_settings['database'],
        server_host     => $influxdb_settings['host'],
        server_port     => $influxdb_settings['port'],
      ...
      class { 'icinga2::feature::influxdb':
        ...
        password => $influxdb_settings['password'],
        database => $influxdb_settings['database'],
        require  => Psick_profile::Influxdb::Grant['icinga2'],
      }
    }
    ...
    # It's possible to customise via the hiera key psick_profile::icinga2::puppetdb_hosts_override_hash
    ...
    # are deep merged with the default ones set via psick_profile::icinga2::host_default_params
    ...
    $config_hash.each |$k,$v| {
      ::psick_profile::icinga2::config { $k:
        * => $v,
      }
    }
