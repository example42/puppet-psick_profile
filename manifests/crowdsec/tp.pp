# psick_profile::crowdsec::tp
#
# @summary This tp profile manages crowdsec with Tiny Puppet (tp)
#
# When you include this class the relevant tp::install define is declared
# which is expected to install crowdsec package and manage its service.
# Via the resources_hash parameter is possible to pass hashes of tp::conf and
# tp::dir defines which can manage crowdsec configuration files and
# whole dirs.
...
#
# @example Just include it to install crowdsec
#   include psick_profile::crowdsec::tp
#
# @example Include via psick module classification (yaml)
#   psick::profiles::linux_classes:
#     crowdsec: psick_profile::crowdsec::tp
#
# @example To use upstream repos instead of OS defaults (if tinydata available) as packages source:
#   psick_profile::crowdsec::tp::upstream_repo: true
#
# @example Manage extra configs via hiera (yaml) with templates based on custom options
#   psick_profile::crowdsec::tp::ensure: present
#   psick_profile::crowdsec::tp::resources:
#     tp::conf:
#       crowdsec:
#         epp: profile/crowdsec/crowdsec.conf.epp
#       crowdsec::dot.conf:
#         epp: profile/crowdsec/dot.conf.epp
#         base_dir: conf
#     exec:
#       crowdsec::setup:
#         command: '/usr/local/bin/crowdsec_setup'
#         creates: '/opt/crowdsec'
#   psick_profile::crowdsec::tp::options:
#     key: value
#
...
#   automatically added.
#   psick_profile::crowdsec::tp::auto_conf: true
#
# @param manage If to actually manage any resource in this profile or not.
# @param ensure If to install or remove crowdsec. Valid values are present, absent, latest
#   or any version string, matching the expected crowdsec package version.
# @param upstream_repo If to use crowdsec upstream repos as source for packages
...
# @param settings_hash An hash of tp settings to override default crowdsec file
...
#
# @param auto_conf If to enable automatic configuration of crowdsec based on the
#   resources_auto_conf_hash and options_auto_conf_hash parameters, if present in
#   data/common/crowdsec.yaml. You can both override them in your Hiera files
...
#   Check psick_profile::crowdsec::tp::resources_auto_conf_hash in
#   data/common/crowdsec.yaml for the auto_conf defaults.
# @param options_auto_conf_hash The default options hash if auto_conf is set.
#   Check psick_profile::crowdsec::tp::options_auto_conf_hash in
#   data/common/crowdsec.yaml for the auto_conf defaults.
#
# @param resources An hash of any resource, like tp::conf, tp::dir, exec or whatever
#   to declare for crowdsec confiuration. Can also come from a third-party
#   component modules with dedicated crowdsec resources.
...
#   parameter.
# @param resource_lookup_method What lookup method to use for psick_profile::crowdsec::tp::resources
# @param resources_defaults An Hash of resources with their default params, to be merged with
#   $resources.
#
# @param auto_prereq If to automatically install eventual dependencies for crowdsec.
#   Set to false if you have problems with duplicated resources, being sure that you
#   manage the prerequistes to install crowdsec (other packages, repos or tp installs).
#
...
#
class psick_profile::crowdsec::tp (
...
#  Hash               $options                 = {},
  ...
  $options=lookup('psick_profile::crowdsec::tp::options', Hash, $options_lookup_method, {})
  ...
  if $manage {
    ...
    # Declaration of psick_profile::crowdsec::tp::resources
    $resources=lookup('psick_profile::crowdsec::tp::resources', Hash, $resources_lookup_method, {})
