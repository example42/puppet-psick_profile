...
class psick_profile::wordpress (
  ...
  if $manage {
    ...
    if $create_default_instance {
      psick_profile::wordpress::instance { 'wordpress':
        * => $instances_defaults,
      }
    }
    $instances.each |$k,$v| {
      psick_profile::wordpress::instance { $k:
