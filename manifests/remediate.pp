...
class psick_profile::remediate (
  ...
  if $manage {
    ...
    psick::tools::create_dir { 'psick_profile::remediate::remediate_dir':
      path  => $remediate_dir,
      owner => $user,
    }
    file { "${remediate_dir}/docker-compose.yml":
      ...
      owner   => $user,
      require => Psick::Tools::Create_dir['psick_profile::remediate::remediate_dir'],
    }
    if $license_json_source {
      ...
      if ! $silence_notify {
        notify { 'psick_profile::remediate::license warning':
